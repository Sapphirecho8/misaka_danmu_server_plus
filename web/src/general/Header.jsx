...
export const Header = () => {
  ...
  const filterByPerm = item => {
    // 更细的权限控制：
    // - SOURCE 需 editScrapers
    // - CONTROL 需 editControl
    // - 其余按 view.* 为默认可见控制
    const mapping = new Map([
      [RoutePaths.HOME, () => perms['view.home'] !== false],
      [RoutePaths.LIBRARY, () => perms['view.library'] !== false],
      [RoutePaths.TASK, () => perms['view.tasks.running'] !== false],
      [RoutePaths.BULLET, () => perms['view.bullet.tokens'] !== false],
      [RoutePaths.SOURCE, () => (userinfo?.username?.toLowerCase?.() === 'admin') || !!perms['editScrapers']],
      [RoutePaths.CONTROL, () => (userinfo?.username?.toLowerCase?.() === 'admin') || !!perms['editControl']],
      [RoutePaths.SETTING, () => perms['view.settings.security'] !== false],
    ])
    const fn = mapping.get(item.key)
    if (!fn) return true
    if ((userinfo?.username || '').toLowerCase() === 'admin') return true
          ...
          <>
            {!it.children?.length ? (
              ...
              <Dropdown
                menu={{
                  items: [
                    ...it.children.filter(filterByPerm).map(o => ({
                      key: o.key,
                      label: (
                        <div
                          key={o.key}
    ...
    <div className="fixed top-0 left-0 w-full shadow-box z-50 py-2 bg-base-bg">
      <div className="flex justify-start items-center max-w-[1200px] mx-auto w-full px-6 gap-4">
        ...
        <div className="flex items-center justify-center">
          {navItems.filter(filterByPerm).map(it => (
            <div
              key={it.key}
              className={classNames(
