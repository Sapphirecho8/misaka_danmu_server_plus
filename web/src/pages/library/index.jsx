...
export const Library = () => {
  ...
  const canEditLibrary = (userinfo?.username || '').toLowerCase() === 'admin' || !!perms.editLibrary
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <Space>
            <Tooltip title={canEditLibrary ? '编辑影视信息' : '无权限'}>
              <span
                className={`cursor-pointer ${canEditLibrary ? 'hover:text-primary' : 'opacity-40 cursor-not-allowed'}`}
                onClick={async () => {
            ...
            <Tooltip title={canEditLibrary ? '详情' : '无权限'}>
              <span
                className={`cursor-pointer ${canEditLibrary ? 'hover:text-primary' : 'opacity-40 cursor-not-allowed'}`}
                onClick={() => {
                  if (!canEditLibrary) return
            ...
            <Tooltip title={canEditLibrary ? '删除' : '无权限'}>
              <span
                className={`cursor-pointer ${canEditLibrary ? 'hover:text-primary' : 'opacity-40 cursor-not-allowed'}`}
                onClick={() => {
                  if (!canEditLibrary) return
