...
export const changePassword = data =>
  api.put('/api/ui/auth/users/me/password', {
...
/** 更新全局每小时流控上限（仅管理员） */
export const setGlobalRateLimit = data => api.put('/api/ui/rate-limit/global-limit', data)
// 注意：api.get 的第二个参数会被作为 axios 的 params 传递，
// 这里应直接传入形如 { windowHours: 1 } 或 { all: true } 的对象，
// 而不是再包一层 { params: {...} }，否则会生成 params[windowHours] 这类无法被后端识别的查询参数。
export const getUsageAggregate = params => api.get('/api/ui/usage/aggregate', params)
/** Token 使用聚合 */
export const getTokenUsageAggregate = params => api.get('/api/ui/tokens/usage/aggregate', params)
...
/** 锁定token（不可逆） */
export const lockToken = data => api.put(`/api/ui/tokens/${data.tokenId}/lock`)
/** 解除token锁定（需权限） */
export const unlockToken = data => api.put(`/api/ui/tokens/${data.tokenId}/unlock`)

/** ----------------- 用户管理（管理员） ----------------- */
export const listUsers = () => api.get('/api/ui/users')
export const createUser = data => api.post('/api/ui/users', data)
export const updateUser = (id, data) => api.put(`/api/ui/users/${id}`, data)
export const deleteUser = id => api.delete(`/api/ui/users/${id}`)
/** 邀请管理 */
export const listInvites = () => api.get('/api/ui/invites')
export const createInvite = data => api.post('/api/ui/invites', data)
export const deleteInviteApi = id => api.delete(`/api/ui/invites/${id}`)
/** 受邀请注册 */
export const registerByInvite = data => api.post('/api/ui/auth/register', data)
/** 校验邀请码（公开） */
export const validateInvite = params => api.get('/api/ui/invites/validate', params)
/** 获取ua规则 */
